...
// void laserCloudHandler(const sensor_msgs::PointCloud2ConstPtr& laserCloudMsg)  ,const pcl::PointCloud<pcl::PointXYZ>& cloud2
void laserCloudHandler(const pcl::PointCloud<pcl::PointXYZ>::Ptr cloud1, pcl::PointCloud<pcl::PointXYZ>::Ptr allSurfPoint)
{
...
//   double timeScanCur = laserCloudMsg->header.stamp.toSec();
  ...
  // std::stringstream ss;
  // ss << tempCount;
  // std::string path = "/home/renxiao/catkin_ws/src/LOAM_NOTED/src/loam_velodyne/data/";
  // pcl::io::savePCDFileASCII (path + ss.str() + ".pcd", laserCloudIn);
...
// pcl::fromROSMsg(*laserCloudMsg, laserCloudIn);
  ...
  pcl::PointCloud<pcl::PointXYZ>::Ptr surfPointsFlat_xyz(new pcl::PointCloud<pcl::PointXYZ>);
  pcl::copyPointCloud(surfPointsLessFlat, *surfPointsFlat_xyz);

  std::cout  << "laserCloudIn.size() = " << laserCloudIn.size() << "   surfPointsLessFlat->size() = " << surfPointsLessFlat.size() << std::endl;

  pcl::copyPointCloud(*surfPointsFlat_xyz, *allSurfPoint);//提取所有的平面点
}


bool findPlaneCorrPoint(const pcl::PointCloud<pcl::PointXYZ>::Ptr cloud_input,
                        const pcl::PointCloud<pcl::PointXYZ>::Ptr cloud_target,
                        const pcl::PointCloud<pcl::PointXYZ>::Ptr surfPointFlat_input,
                        const pcl::PointCloud<pcl::PointXYZ>::Ptr surfPointFlat_target)
{
    std::stringstream ss;

    pcl::registration::CorrespondenceEstimation<pcl::PointXYZ, pcl::PointXYZ> core;
    core.setInputSource(surfPointFlat_input);//quary
    core.setInputTarget(surfPointFlat_target);//match
    pcl::Correspondences all_correspondences;
    core.determineReciprocalCorrespondences(all_correspondences);

    boost::shared_ptr<pcl::visualization::PCLVisualizer> viewer(new pcl::visualization::PCLVisualizer("Cloud Viewer"));
    pcl::visualization::PointCloudColorHandlerCustom<pcl::PointXYZ> input_cloud_color(surfPointFlat_input, 0, 255, 0);
    pcl::visualization::PointCloudColorHandlerCustom<pcl::PointXYZ> target_cloud_color(surfPointFlat_target, 255, 0, 0);
    viewer->setBackgroundColor(0, 0, 0);   
    viewer->addPointCloud<pcl::PointXYZ>(surfPointFlat_input,input_cloud_color,"cloud_input");
    viewer->addPointCloud<pcl::PointXYZ>(surfPointFlat_target, target_cloud_color, "cloud_target");
    ...
    viewer->addCorrespondences<pcl::PointXYZ>(surfPointFlat_input, surfPointFlat_target, all_correspondences, "correspondence");
    viewer->setShapeRenderingProperties(pcl::visualization::PCL_VISUALIZER_LINE_WIDTH, 2.5, "correspondence");
    viewer->setShapeRenderingProperties(pcl::visualization::PCL_VISUALIZER_COLOR, 1, 1, 1, "correspondence");

    // std::cout << "cloud_input->size() = " << cloud_input->size() << std::endl;
    // std::cout << "cloud_target->size() = " << cloud_target->size() << std::endl;
    // std::cout << "surfPointFlat_input->size() = " << surfPointFlat_input->size() << std::endl;
    // std::cout << "surfPointFlat_input->size() = " << surfPointFlat_target->size() << std::endl;
...
int main(int argc, char** argv)
{
  ...
  std::vector<int> inputCloudIdx;
  std::vector<int> targetCloudIdx;

  std::string file_name_1 = "/home/renxiao/catkin_ws/src/LOAM_NOTED/src/loam_velodyne/data/500.pcd";
  std::string file_name_2 = "/home/renxiao/catkin_ws/src/LOAM_NOTED/src/loam_velodyne/data/501.pcd";
  ...
  pcl::PointCloud<pcl::PointXYZ>::Ptr surfPointFlat_input(new pcl::PointCloud<pcl::PointXYZ>);
  pcl::PointCloud<pcl::PointXYZ>::Ptr surfPointFlat_target(new pcl::PointCloud<pcl::PointXYZ>);

  laserCloudHandler(cloud_input,surfPointFlat_input);
  laserCloudHandler(cloud_target,surfPointFlat_target);

  findPlaneCorrPoint(cloud_input,cloud_target,surfPointFlat_input,surfPointFlat_target);
